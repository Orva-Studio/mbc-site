...
export default {
  async fetch(request: Request, env: Environment): Promise<Response> {
    ...
    try {
      ...
      const { default: entries, loadModule, INTERNAL_setAllEnv, INTERNAL_setPlatformDataLoader, loadPlatformData } = await import('../dist/entries.js');
...
async function serveHTML(request: Request, entries: any): Promise<Response> {
  // Import the static HTML from the build
  const { publicIndexHtml } = await import('../dist/entries.js');
